  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        ...
        final role = snapshot.data!.getInt('accountType') ?? 0;
        final isCharity = role == 0;

        return BlocProvider(
          create: (_) => ReceivedApplicationsCubit(
            ReceivedApplicationsService(
              dio: DioClient.dio,
              baseEndpoint: isCharity
                  ? '/api/v1/charity/applications'
                  : '/api/v1/donor-organization/offer-applications',
              idKey: isCharity ? 'needApplicationId' : 'offerApplicationId',
            ),
          )..fetchApplications(),
